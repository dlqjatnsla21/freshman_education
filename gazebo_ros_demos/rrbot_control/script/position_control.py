...
def inverse_kinematics(x, y, z, up_down):
    ...
    print("Target Joint Values :", round(theta1*180/m.pi, 3), round(theta2*180/m.pi, 3), round(theta3*180/m.pi, 3))
...
while not rospy.is_shutdown():
    x, y, z, s = input("Enter the target position and the solution type (U for upper, L for lower) : ").split()
    if s == 'U':
        up = True
    elif s == 'L':
        up = False
    else:
        print("Wrong solution! Type it again.")
        continue
    ...
    if first_callback:
        if initial_state_read:
            ...
            print("       initial_pose :", round(th1_i*180/m.pi, 3), round(th2_i*180/m.pi, 3), round(th3_i*180/m.pi, 3))
            for i in range(step):
